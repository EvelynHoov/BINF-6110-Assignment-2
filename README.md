...
- using Illumina reads from (insert initial paper)
- quality of reads will be evaluated using fastqc to determine if a quality control step will be needed
- aligned to salmon, a pseudoaligner, which is used because a reference transcriptome is available, gene counts only are alright, a quantification step is not required, and it is faster than a splice-aware aligner
- differential expression analysis will be done using DESeq2 in RStudio as a small sample size (n=3) is used with few random effects
## Methods  
## Results  
## Discussion  
